    ...
    while total_iters <  opt.total_num_giters:
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.print_freq == 0:    # print training losses and save logging information to the disk
                ...
                #time_used.append(t_comp)
                #time_used_arr=np.array(time_used)
                #time_mean=np.mean(time_used_arr)
                #time_std=np.std(time_used_arr)
                #print("time_mean:%.3f; time_std:%.3f"%(time_mean,time_std))
                #print(time_used)
