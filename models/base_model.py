...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        if self.opt.use_pytorch_scores and self.opt.score_name is not None:
            print('torch version inception_metrics init ...')
            no_FID = True
            no_IS = True
            ...
            print('tensorflow version inception_metrics init ...')
            for name in self.opt.score_name:
                if name == 'FID':
            ...
            print('tensorflow version inception_metrics init finished')
    ...
    def get_current_losses(self):
        ...
        if self.opt.model == 'cdegan':
            G_fitness = getattr(self, 'G_Fitness')
            D_fitness = getattr(self, 'D_Fitness')
            errors_ret['Fq'] = G_fitness[0][0]
            errors_ret['Fd'] = G_fitness[0][1]
            errors_ret['F'] = G_fitness[0][2]
            errors_ret['D1_Fd'] = D_fitness[0]
            if self.opt.d_candi_num == 2:
                errors_ret['D2_Fd'] = D_fitness[1]
        return errors_ret
        
    def get_fit_and_mut(self):
        """Return traning losses / errors. train.py will print out these errors on console, and save them to a file"""
        save_D_fitness_score = os.path.join(self.save_dir, 'D_fitness_score.txt')
        save_D_selected_mutation = os.path.join(self.save_dir, 'D_slected_mutation.txt')
        save_G_fitness_score = os.path.join(self.save_dir, 'G_fitness_score.txt')
        save_G_selected_mutation = os.path.join(self.save_dir, 'G_slected_mutation.txt')
        D_Fitness = getattr(self, 'D_Fitness')
        d_sel_mut = getattr(self, 'd_sel_mut')
        G_Fitness = getattr(self, 'G_Fitness')
        g_sel_mut = getattr(self, 'g_sel_mut')
        with open(save_D_fitness_score,'a') as f:
            np.savetxt(f, D_Fitness, delimiter=',',newline='\n')
        with open(save_D_selected_mutation,'a') as q:
            np.savetxt(q, d_sel_mut,delimiter=',',newline='\n',fmt ='%s')
        with open(save_G_fitness_score,'a') as d:
            np.savetxt(d, G_Fitness, delimiter=',',newline='\n')
        with open(save_G_selected_mutation,'a') as e:
            np.savetxt(e, g_sel_mut,delimiter=',',newline='\n',fmt ='%s')
    ...
    def get_current_scores(self):
        ...
        if self.opt.use_pytorch_scores:
            print('torch version IS calculating')
            ...
            print('tensorflow version IS calculating')
